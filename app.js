...
bot.on("guildMemberAdd", async member => {
  ...
  .setColor(8388608)
  .setDescription(`Welcome to the Villoux Server System ${member} (${member.user.tag})!\nTo verify yourself, please follow the instructions below.`)
...
bot.on("message", async message => {
  if (!["text"].includes(message.channel.type)) return;
  ...
  if (command === "verify") {
    ...
    if (!countries.includes(args[1].toUpperCase())) {
      return message.channel.send("Please use your country's two-letter code.  A list of codes can be found here - https://goo.gl/7EkkeF");
    }
    message.guild.fetchMember(message.author).then(m => m.removeRole(message.guild.roles.get("355574470062243850")));
    ...
    message.author.send(`Thank you for the information.  Your account has now been verified on our Discord server.  To familiarize yourself with Villoux, I might recommend taking a look around our website - http://www.villoux.com/.  If you are ever in need of any further assistance, please contact the Villoux Chief of Staff, Dr. Baskerville, either by direct message or email at drbaskerville@villoux.com`)
  }
  ...
  if (command === "edit") {
    ...
    deleteBotMsg(`Successfully set the join message to \`\`\`${args.joinFrom(0)}\`\`\``);
  }
